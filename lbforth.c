/* lbforth.c - A minimal FORTH compiler in ANSI C */
/* Project: SUBLEQ */
/* Date: Sunday, April 9, 2023 */
/* See also: [[https://gist.github.com/lbruder/10007431]] */
...
BUILTIN(71, "WORDS", words, 0)
{
  cell i, len, ret;
  char buf[32], *s;
  for (ret = *latest; ret; ret = readMem(ret)){
    /* if (memory[ret + CELL_SIZE] & FLAG_HIDDEN) continue; */
    len = memory[ret + CELL_SIZE] & MASK_NAMELENGTH;
    for( s=buf, i=0; i<len; i++ ){
      *s++ = memory[ret + i + 1 + CELL_SIZE];
    }
    *s++ =  ' '; *s = 0;
    tell( buf );
  }
